...
class Omnivo_Calendar_DB
{
    public static function getEventHours($args)
    {
		...
		FROM 
			' . $wpdb->prefix . 'omnivo_calendar_event_hours AS eh
		LEFT JOIN
			' . $wpdb->prefix . 'omnivo_calendar_event_hours_booking AS ehb ON(ehb.event_hours_id=eh.event_hours_id)
		LEFT JOIN
		...
		LEFT JOIN
			...
			FROM ' . $wpdb->prefix . 'omnivo_calendar_event_hours_booking 
		...
		LEFT JOIN
			...
			FROM ' . $wpdb->prefix . 'omnivo_calendar_event_hours_booking AS b 
			LEFT JOIN ' . $wpdb->prefix . 'omnivo_calendar_guests AS g ON (b.guest_id=g.guest_id)
			WHERE g.email="%s" 
    ...
    public static function getBookings($args)
    {
		...
		FROM 
			' . $wpdb->prefix . 'omnivo_calendar_event_hours_booking AS booking
		LEFT JOIN 
			' . $wpdb->prefix . 'omnivo_calendar_event_hours AS event_hour 
		...
		LEFT JOIN 
			' . $wpdb->prefix . 'omnivo_calendar_guests AS guest
    ...
    public static function deleteBooking($booking_id)
    {
        ...
        $query = $wpdb->prepare('DELETE FROM `' . $wpdb->prefix . 'omnivo_calendar_event_hours_booking` WHERE booking_id=%d', $booking_id);
    ...
    public static function createBooking($args)
    {
        ...
        $query = 'INSERT INTO ' . $wpdb->prefix . 'omnivo_calendar_event_hours_booking(event_hours_id, user_id, booking_datetime, validation_code, guest_id) 
				VALUES (%d, %d, %s, %s, %d)';
    ...
    public static function createGuest($args)
    {
        ...
        $query .=
            'INSERT INTO ' . $wpdb->prefix . 'omnivo_calendar_guests(name, email, phone, message) VALUES (%s, %s, %s, %s)';
