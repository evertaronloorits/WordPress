<?php
class omnivo_calendar_upcoming_events_widget extends WP_Widget
{
    /** constructor */
    function __construct()
    {
        $widget_options = array(
            'classname' => 'omnivo_calendar_upcoming_events_widget',
            'description' => 'Displays upcoming events scrolling list'
        );

        parent::__construct('omnivo_calendar_upcoming_events', __('Upcoming Events', 'omnivo_calendar'), $widget_options);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $query = "SELECT TIME_FORMAT(t1.start, '%H.%i') AS start, TIME_FORMAT(t1.end, '%H.%i') AS end, t1.event_id AS event_id, t1.before_hour_text AS description1, t1.after_hour_text AS description2, t2.post_title, t2.post_name, t3.post_title as weekday FROM ".$wpdb->prefix."omnivo_calendar_event_hours AS t1 
        ...
        $output = '';
        $output .= '<div class="omnivo_calendar_upcoming_events_widget">';
        if($title)
        {
            if($title_color!="")
        ...
        $output .= '<div class="omnivo_calendar_upcoming_events_wrapper page_margin_top">';
        if($event_hours_count):
            $output .= '<ul class="omnivo_calendar_upcoming_events clearfix autoscroll-' . (int)$auto_scroll . '">';
            for($i=0; $i<$event_hours_count; $i++)
            {
                ...
                {
                    $event_color = get_post_meta($event_hours[$i]->event_id, "omnivo_calendar_color", true);
            ...
            $output .= '</ul><div class="omnivo_calendar_upcoming_event_controls">
			<a href="#" id="upcoming_event_prev"><span class="omnivo_calendar_upcoming_event_prev_arrow"></span></a>
			<a href="#" id="upcoming_event_next"><span class="omnivo_calendar_upcoming_event_next_arrow"></span></a>
		</div>';
        else:
            $output .= '<p class="message">' . sprintf(__('No upcoming %s for today' , 'omnivo_calendar'), strtolower($omnivo_calendar_events_settings['label_plural'])) . '</p>';
    ...
    function form($instance)
    {
        ...
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'omnivo_calendar'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('title_color'); ?>"><?php _e('Title color', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Count', 'omnivo_calendar'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('count'); ?>" name="<?php echo $this->get_field_name('count'); ?>" type="text" value="<?php echo $count; ?>" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('display_settings'); ?>"><?php _e('Display settings', 'omnivo_calendar'); ?></label>
            <select id="<?php echo $this->get_field_id('display_settings'); ?>" name="<?php echo $this->get_field_name('display_settings'); ?>">
                <option value="today"<?php echo ($display_settings=="today" ? " selected='selected'" : ""); ?>><?php _e('today upcoming', 'omnivo_calendar'); ?></option>
                <option value="all"<?php echo ($display_settings=="all" ? " selected='selected'" : ""); ?>><?php _e('all upcoming', 'omnivo_calendar'); ?></option>
                <option value="current"<?php echo ($display_settings=="current" ? " selected='selected'" : ""); ?>><?php _e('current events', 'omnivo_calendar'); ?></option>
            </select>
        </p>
        <p <?php echo ($display_settings=="today" || $display_settings=="current" || $display_settings=="" ? ' style="display: none;"' : ''); ?>>
            <label for="<?php echo $this->get_field_id('within_next'); ?>"><?php _e('All upcoming within next', 'omnivo_calendar'); ?></label>
            <input class="widefat" style="width: 60px;" id="<?php echo $this->get_field_id('within_next'); ?>" name="<?php echo $this->get_field_name('within_next'); ?>" type="text" value="<?php echo $within_next; ?>" /> <?php _e("day(s)", 'omnivo_calendar'); ?>
        </p>
        <p>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('time_mode'); ?>"><?php _e('Time from', 'omnivo_calendar'); ?></label>
            <select id="upcoming_events_time_from" name="<?php echo $this->get_field_name('time_mode'); ?>">
                <option value="server"<?php echo ($time_mode=="server" ? " selected='selected'" : ""); ?>><?php _e('server', 'omnivo_calendar'); ?></option>
                <option value="database"<?php echo ($time_mode=="database" ? " selected='selected'" : ""); ?>><?php _e('database', 'omnivo_calendar'); ?></option>
            </select>
        </p>
        <p class="upcoming_events_timezone_row" <?php echo ($time_mode=="database" ? " style='display: none;'" : ""); ?>>
            <label for="<?php echo $this->get_field_id('timezone'); ?>"><?php _e('Timezone', 'omnivo_calendar'); ?></label>
            <select name="<?php echo $this->get_field_name('timezone'); ?>">
                <option value="localtime"<?php echo ($timezone=="localtime" ? " selected='selected'" : ""); ?>><?php _e('localtime', 'omnivo_calendar'); echo " (now: " .  date('H:i:s', current_time('timestamp')) . ")"; ?></option>
                <option value="utc"<?php echo ($timezone=="utc" ? " selected='selected'" : ""); ?>><?php _e('utc', 'omnivo_calendar'); echo " (now: " .  date('H:i:s', current_time('timestamp', 1)) . ")"; ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('categories'); ?>"><?php _e('Categories', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('hour_categories'); ?>"><?php _e('Hour categories', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('auto_scroll'); ?>"><?php _e('Auto scroll', 'omnivo_calendar'); ?></label>
            <select id="<?php echo $this->get_field_id('auto_scroll'); ?>" name="<?php echo $this->get_field_name('auto_scroll'); ?>">
                <option value="0"<?php echo ((int)$auto_scroll==0 ? " selected='selected'" : ""); ?>><?php _e('no', 'omnivo_calendar'); ?></option>
                <option value="1"<?php echo ((int)$auto_scroll==1 ? " selected='selected'" : ""); ?>><?php _e('yes', 'omnivo_calendar'); ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('custom_url'); ?>"><?php _e('Custom event url', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('event_featured_image'); ?>"><?php _e('Event featured image', 'omnivo_calendar'); ?></label>
            <select id="<?php echo $this->get_field_id('event_featured_image'); ?>" name="<?php echo $this->get_field_name('event_featured_image'); ?>">
                <option value="0"<?php echo ((int)$event_featured_image==0 ? " selected='selected'" : ""); ?>><?php _e('Hide', 'omnivo_calendar'); ?></option>
                <option value="1"<?php echo ((int)$event_featured_image==1 ? " selected='selected'" : ""); ?>><?php _e('Show', 'omnivo_calendar'); ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('disable_url'); ?>"><?php _e('Disable event url', 'omnivo_calendar'); ?></label>
            <select id="<?php echo $this->get_field_id('disable_url'); ?>" name="<?php echo $this->get_field_name('disable_url'); ?>">
                <option value="0"<?php echo ((int)$disable_url==0 ? " selected='selected'" : ""); ?>><?php _e('no', 'omnivo_calendar'); ?></option>
                <option value="1"<?php echo ((int)$disable_url==1 ? " selected='selected'" : ""); ?>><?php _e('yes', 'omnivo_calendar'); ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('background_color'); ?>"><?php _e('Background color', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('hover_background_color'); ?>"><?php _e('Hover background color', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('text_color'); ?>"><?php _e('Text color', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('hover_text_color'); ?>"><?php _e('Hover text color', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('item_border_color'); ?>"><?php _e('Item border color', 'omnivo_calendar'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('hover_item_border_color'); ?>"><?php _e('Hover item border color', 'omnivo_calendar'); ?></label>
...
//register widget
function omnivo_calendar_upcoming_events_widget_init()
{
    return register_widget("omnivo_calendar_upcoming_events_widget");
}
add_action('widgets_init', 'omnivo_calendar_upcoming_events_widget_init');
?>