<?php
function omnivo_calendar_remove_wpautop($content)
{
    return do_shortcode(shortcode_unautop($content));
}

//items list
function omnivo_calendar_event_items_list($atts, $content)
{
    ...
    $output = '';
    $output .= '
	<ul class="omnivo_calendar_event_items_list' . ($class!='' ? ' ' . $class : '') . '">
		' . omnivo_calendar_remove_wpautop($content) . '
	</ul>';
    return $output;
}
add_shortcode("omnivo_calendar_items_list", "omnivo_calendar_event_items_list");

//items list
function omnivo_calendar_event_item($atts, $content)
{
    extract(shortcode_atts(array(
        "type" => "",
        "border_color" => "",
    ...
    $output = '';
    $output .= '
	<li' . ($type=="info" ? ' class="omnivo_calendar_clearfix type_info"' : '') . ($border_color!='' ? ' style="border-bottom: ' . ($border_color=='none' ? 'none' : '1px solid #' . $border_color . '') . ';"' : '') . '>
		<' . ($type=="info" ? 'label' : 'span') . ($text_color!='' ? ' style="color: #' . $text_color . ';"' : '') . '>' . omnivo_calendar_remove_wpautop($content) . '</' . ($type=="info" ? 'label' : 'span') . '>';
    if($value!="")
        $output .= '<div class="omnivo_calendar_event_text">' . $value . '</div>';
    $output .= '
	</li>';
    return $output;
}
add_shortcode("omnivo_calendar_item", "omnivo_calendar_event_item");

//columns
function omnivo_calendar_event_columns($atts, $content)
{
    extract(shortcode_atts(array(
        "class" => ""
    ), $atts));
    return '<div class="omnivo_calendar_event_columns' . ($class!='' ? ' ' . $class : '') . '">' . omnivo_calendar_remove_wpautop($content) . '</div>';
}
add_shortcode("omnivo_calendar_columns", "omnivo_calendar_event_columns");

//column left
function omnivo_calendar_event_column_left($atts, $content)
{
    return '<div class="omnivo_calendar_event_column_left">' . omnivo_calendar_remove_wpautop($content) . '</div>';
}
add_shortcode("omnivo_calendar_column_left", "omnivo_calendar_event_column_left");

//column right
function omnivo_calendar_event_column_right($atts, $content)
{
    return '<div class="omnivo_calendar_event_column_right">' . omnivo_calendar_remove_wpautop($content) . '</div>';
}
add_shortcode("omnivo_calendar_column_right", "omnivo_calendar_event_column_right");

//event hours
function omnivo_calendar_event_hours($atts, $content)
{
    global $post;
    ...
    if($title!="")
        $output .= '<h3 class="omnivo_calendar_event_margin_top_27">' . $title . '<span class="omnivo_calendar_event_hours_count">(' . $event_hours_count . ')</span></h3>';

    //custom styles
    if(strtoupper($available_slots_color)!="34495E")
    {
        $inline_style .= (strtoupper($available_slots_color)!="34495E" ? ' .omnivo_calendar_event_hours .available_slots_wrapper span.available_slots { color:#' . $available_slots_color . ' !important;}' : '');
    }

    $output .= '
	<ul id="event_hours_list" class="omnivo_calendar_clearfix omnivo_calendar_event_hours' . ($class!="" ? ' ' . $class : '') . '">';
    for($i=0; $i<$event_hours_count; $i++)
    {
        //get event color
        if($border_color=="")
            $border_color = "#" . get_post_meta($event_hours[$i]->event_id, "omnivo_calendar_color", true);
        ...
        {
            $output .= '<p' . ($text_color!="" ? ' style="color:' . $text_color . ';"' : '') . ' class="omnivo_calendar_event_padding_bottom_0">';
        ...
        if($show_booking_button=='yes')
        {
            ...
            $booking_content = prepare_booking_button(array(
                'omnivo_calendar_page_id' => $omnivo_calendar_page_id,
                'redirect' => 'yes',
...
add_shortcode("omnivo_calendar_event_hours", "omnivo_calendar_event_hours");
?>