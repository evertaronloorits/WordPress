...
<div class='omnivo_calendar_btn_wrapper'>{omnivo_calendar_btn_book}{omnivo_calendar_btn_cancel}</div>");
...
<div class='omnivo_calendar_btn_wrapper'>{omnivo_calendar_btn_continue}</div>");

function omnivo_calendar_calendar($atts, $content)
{
    $omnivo_calendar_events_settings = omnivo_calendar_events_settings();
    wp_register_style('omnivo_calendar_inline_style', false);
    wp_enqueue_style('omnivo_calendar_inline_style');
    $inline_style = '';
    ...
    if(isset($atts['shortcode_id']) && strlen($atts['shortcode_id']))
    {
        ...
        if($omnivo_calendar_shortcodes_list!==false && !empty($omnivo_calendar_shortcodes_list[$atts['shortcode_id']]))
        {
            $shortcode = $omnivo_calendar_shortcodes_list[$atts['shortcode_id']];
    ...
    if(count($event_category_array))
    {
        ...
        $events_array_cat = get_posts(array(
            ...
            'orderby' => 'menu_order',
            'order' => 'ASC',
            'events_category' => implode("','", array_map("omnivo_calendar_strtolower_urlencode", $event_category_array))
        ));
    ...
    $output .= "<div " . ($id!="" ? "id='".urlencode($id)."'" : "") . " class='omnivo_calendar_wrapper " . ($direction=="rtl" ? "rtl" : "") . " " . ($show_booking_button=="on_hover" ? "booking_hover_buttons" : "") . "'>";

    if($filter_style=="dropdown_list")
    {
        $output .= '<div class="omnivo_calendar_navigation_wrapper ' . ($filter_kind=="event_and_event_category" ? "omnivo_calendar_double_buttons" : "") . '">
		<div class="omnivo_calendar_navigation_cell omnivo_calendar_clearfix">';

        if($filter_kind=="event_category" || $filter_kind=="event_and_event_category")
        {
            $output .= '<ul class="omnivo_calendar_clearfix tabs_box_navigation events_categories_filter' . ((int)$responsive ? " omnivo_calendar_responsive" : "") . ' sf-omnivo_calendar-menu">
        ...
        {
            $output .= '<ul class="omnivo_calendar_clearfix tabs_box_navigation events_filter' . ((int)$responsive ? " omnivo_calendar_responsive" : "") . ' sf-omnivo_calendar-menu">
        ...
        if($export_to_pdf_button && $responsive)
        {
            ...
            $output .=
                "<div class='omnivo_calendar_navigation_cell omnivo_calendar_clearfix'>
					<form class='omnivo_calendar_generate_pdf' action='' method='post'>
						<textarea class='omnivo_calendar_pdf_html' name='omnivo_calendar_pdf_html_content'></textarea>
						<input type='hidden' name='omnivo_calendar_action' value='omnivo_calendar_generate_pdf'/>
                        <input type='hidden' name='omnivo_calendar_pdf_font' value='" . $pdf_font . "'/>
    ...
    {
        $inline_style .= $custom_css . ((int)$row_height!=31 ? ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar td{height: ' . (int)$row_height . (substr($row_height, -2)!="px" ? 'px' : '') . ';}' : '') . (strtoupper($box_bg_color)!="00A27C" ? ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar .event{background: #' . $box_bg_color . ';}' : '') . (strtoupper($filter_color)!="00A27C" ? ($id!="" ? '#' . $id : '') . ' .omnivo_calendar_tabs_navigation li a:hover,' . ($id!="" ? '#' . $id : '') . ' .omnivo_calendar_tabs_navigation li a.selected,' . ($id!="" ? '#' . $id : '') . ' .omnivo_calendar_tabs_navigation li.ui-tabs-active a{border-color:#' . $filter_color . ' !important;}' . ($id!="" ? '.' . $id : '') . '.tabs_box_navigation.sf-omnivo_calendar-menu .tabs_box_navigation_selected{background-color:#' . $filter_color . ';border-color:#' . $filter_color . ';}' . ($id!="" ? '.' . $id : '') . '.tabs_box_navigation.sf-omnivo_calendar-menu .tabs_box_navigation_selected:hover{background-color: #FFF; border: 1px solid rgba(0, 0, 0, 0.1);}' . ($id!="" ? '.' . $id : '') . '.sf-omnivo_calendar-menu li ul li a:hover, .sf-omnivo_calendar-menu li ul li.selected a:hover{background-color:#' . $filter_color . ';}' : '') . (strtoupper($available_slots_color)!="FFD544" ? ($id!="" ? '#' . $id : '') . ' .omnivo_calendar .event span.available_slots,' . ($id!="" ? '#' . $id : '') . ' .omnivo_calendar_responsive .omnivo_calendar.small .omnivo_calendar_items_list span.available_slots,' . ($id!="" ? '#' . $id : '') . ' .omnivo_calendar_responsive .omnivo_calendar.small .omnivo_calendar_items_list span.available_slots span.count { color:#' . $available_slots_color . ' !important;}' : '');
    }
    if($font!="")
        wp_enqueue_style('omnivo_calendar_shortcode_fonts','//fonts.googleapis.com/css?family=' . $font . '&amp;subset=' . $font_subset );
    if($font_custom!="" || $font!="" || (int)$font_size>0)
    {
        $font_explode = explode(":", $font);
        $font = '"' . $font_explode[0] . '"';

        $inline_style .= ($font_custom!="" || $font!="" ? ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar, #omnivo_calendar_booking_popup_message .omnivo_calendar_booking_message, #omnivo_calendar_booking_popup_message h2{font-family:' . ($font_custom!="" ? $font_custom : $font) . ' !important;}' : '') . ((int)$font_size>0 ? ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar th,' . ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar td,' . ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar .event .before_hour_text,' . ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar .event .after_hour_text,' . ($id!="" ? '#' . $id : '') . '.omnivo_calendar_tabs .omnivo_calendar .event .event_header{font-size:' . (int)$font_size . 'px !important;}' : '');
    }
    $output .= '<div class="omnivo_calendar_clearfix omnivo_calendar_tabs' . ((int)$responsive ? " omnivo_calendar_responsive" : "") . " event_layout_" . $event_layout . '"' . ($id!="" ? ' id="' . $id . '"' : '') . '>';

    $output .= '<div class="omnivo_calendar_navigation_wrapper ' . ($filter_style=='tabs' ? '' : 'omnivo_calendar_hide') . '">
		<div class="omnivo_calendar_navigation_cell omnivo_calendar_clearfix">';
    ...
    if($filter_kind=="event_and_event_category")
    {
        ...
        $output .= '<ul class="omnivo_calendar_clearfix omnivo_calendar_tabs_navigation all_filters" style="display: none !important;">' . $all_filters_list_html . '</ul>';
    }
    ...
    if($filter_kind=="event" || $filter_kind=="event_and_event_category")
    {
        ...
        $output .= '<ul class="omnivo_calendar_clearfix omnivo_calendar_tabs_navigation events_filter"' . ($filter_style=="dropdown_list" ? ' style="display: none;"' : '') . '>' . $events_list_html_view_all . $events_list_html . '</ul>';
    }
    if($filter_kind=="event_category" || $filter_kind=="event_and_event_category")
    {
        ...
        $output .= '<ul class="omnivo_calendar_clearfix omnivo_calendar_tabs_navigation events_categories_filter"' . ($filter_style=="dropdown_list" ? ' style="display: none;"' : '') . '>' .  $events_categories_list_html_view_all . $events_categories_list_html . '</ul>';
    }
    ...
    if(!$hide_all_events_view)
    {
        $output .= '<div id="all-events' . ($id!='' ? '-' . urlencode($id) : '') . '">' . (empty($events_array_verified) ? omnivo_calendar_get_omnivo_calendar($atts, $events_array) : ($events_array_verified!=-1 ? omnivo_calendar_get_omnivo_calendar($atts, $events_array_verified) : sprintf(__('No %s available!' , 'omnivo_calendar'), strtolower($omnivo_calendar_events_settings['label_plural'])))) . '</div>';
    }
    ...
    if($filter_kind=="event" || !count($event_category_array) || $filter_kind=="event_and_event_category")
    {
        for($i=0; $i<$events_array_count; $i++)
        {
            $post = get_page_by_path($events_array[$i], ARRAY_A, $omnivo_calendar_events_settings['slug']);
            $categories = wp_get_post_terms($post["ID"], "events_category");
            $categories_str = "";
            foreach($categories as $category)
                $categories_str .= "omnivo_calendar-event-category-" . $category->slug . ($id!='' ? '-' . urlencode($id) : '') . " ";
            $output .= '<div id="' . urlencode($events_array[$i]) . ($id!='' ? '-' . urlencode($id) : '') . '" class="omnivo_calendar-ui-tabs-hide ' . $categories_str . '">' . (empty($events_array_verified) || ($events_array_verified!=-1 && in_array($events_array[$i], $events_array_verified)) ? omnivo_calendar_get_omnivo_calendar($atts, $events_array[$i]) : sprintf(__('No %s available!' , 'omnivo_calendar'), strtolower($omnivo_calendar_events_settings['label_plural']))) . '</div>';
        }
    }
    if($filter_kind=="event_category" || $filter_kind=="event_and_event_category")
    {
        for($i=0; $i<$events_category_array_count; $i++)
        {
            ...
            $events_array_for_omnivo_calendar = array();
            for($j=0; $j<count($events_array_posts); $j++)
                $events_array_for_omnivo_calendar[] = urldecode($events_array_posts[$j]->post_name);
            $output .= '<div id="' . urlencode($event_category_array[$i]) . ($id!='' ? '-' . urlencode($id) : '') . '" class="omnivo_calendar-ui-tabs-hide">' . (count($events_array_posts) ? omnivo_calendar_get_omnivo_calendar($atts, $events_array_for_omnivo_calendar) : sprintf(__('No %1$s available in %2$s category!', 'omnivo_calendar'), strtolower($omnivo_calendar_events_settings['label_plural']), $event_category_array[$i])) . '</div>';
        }
    }
    $output .= '</div>';

    $output .= "<div class='omnivo_calendar_error_message omnivo_calendar_hide'>" . sprintf(__('No %s available!' , 'omnivo_calendar'), strtolower($omnivo_calendar_events_settings['label_plural'])) . "</div>";

    $output .=
        "<div class='omnivo_calendar_booking_overlay omnivo_calendar_hide'>
	</div>
	<div class='omnivo_calendar_booking omnivo_calendar_hide'>
		<div class='omnivo_calendar_booking_message_wrapper'>
			<div class='omnivo_calendar_booking_message' data-event-hour-id>
			</div>
			<div class='omnivo_calendar_preloader omnivo_calendar_hide'>
    ...
    $output .= '<input type="hidden" class="omnivo_calendar_atts" name="omnivo_calendar_atts" value="' . htmlentities(json_encode($atts2)) . '" />';
    $output .= "</div>";

    wp_add_inline_style('omnivo_calendar_inline_style', $inline_style);
    return $output;
}
add_shortcode("omnivo_calendar", "omnivo_calendar_calendar");

/**
 * Generates the Calendar HTML code
 */
function omnivo_calendar_get_omnivo_calendar($atts, $event = null)
{
    $omnivo_calendar_events_settings = omnivo_calendar_events_settings();
    ...
    if($columns!="")
    {
        ...
        foreach($weekdays_explode as $weekday_explode)
            $weekdays_in_query .= "'" . omnivo_calendar_strtolower_urlencode($weekday_explode) . "'" . ($weekday_explode!=end($weekdays_explode) ? "," : "");
    }
	...
	FROM " . $wpdb->prefix . "omnivo_calendar_event_hours AS t1 
		LEFT JOIN {$wpdb->posts} AS t2 ON t1.event_id=t2.ID 
		LEFT JOIN {$wpdb->posts} AS t3 ON t1.weekday_id=t3.ID 
		LEFT JOIN (SELECT event_hours_id, COALESCE(COUNT(booking_id),0) as booking_count FROM " . $wpdb->prefix . "omnivo_calendar_event_hours_booking GROUP BY event_hours_id) AS t4 ON t1.event_hours_id=t4.event_hours_id 
		LEFT JOIN (SELECT event_hours_id, user_id, COUNT(booking_id) as booking_count FROM " . $wpdb->prefix . "omnivo_calendar_event_hours_booking where user_id= " . (int)$user_id . " and user_id!=0 GROUP BY event_hours_id) AS t6 ON t1.event_hours_id=t6.event_hours_id ";
    ...
    if(is_array($event) && count($event))
        $query .= "
			AND t2.post_name IN('" . implode("','", array_map("omnivo_calendar_strtolower_urlencode", $event)) . "')";
    else if($event!=null)
        $query .= "
			AND t2.post_name='" . omnivo_calendar_strtolower_urlencode($event) . "'";
    if($hour_category!=null && $hour_category!="-")
        $query .= "
			AND t1.category IN('" . join("','", $hour_category) . "')";
    $query .= "
			AND 
			t3.post_type='omnivo_weekdays'
			AND
    ...
    if(!count($event_hours))
        return sprintf(__('No %s hours available!' , 'omnivo_calendar'), strtolower($omnivo_calendar_events_settings['label_plural']));
    ...
    $query = "SELECT min(TIME_FORMAT(t1.start, '%H.%i')) AS min, max(REPLACE(TIME_FORMAT(t1.end, '%H.%i'), '00.00', '24.00')) AS max FROM " . $wpdb->prefix . "omnivo_calendar_event_hours AS t1
			LEFT JOIN {$wpdb->posts} AS t2 ON t1.event_id=t2.ID 
			LEFT JOIN {$wpdb->posts} AS t3 ON t1.weekday_id=t3.ID 
			WHERE 
			t2.post_type='" . $omnivo_calendar_events_settings['slug'] . "'
    ...
    {
        $output .= '<table class="omnivo_calendar">
					<thead>
        ...
        for($i=$hours_min; $i<$max_hour; $i=$i+$increment)
        {
            ...
            for($j=0; $j<count($weekdays); $j++)
            {
                ...
                {
                    if(omnivo_calendar_hour_in_array($i, (isset($event_hours_tt[$weekday_fixed_number]) ? $event_hours_tt[$weekday_fixed_number] : array()), $measure, $hours_min))
                    {
                        $rowspan = omnivo_calendar_get_rowspan_value($i, $event_hours_tt[$weekday_fixed_number], 1, $measure, $hours_min);
                        if($rowspan>1)
                        {
                            if((int)$measure==1)
                            {
                        ...
                        {
                            $color = get_post_meta($events[key($events)]["id"], "omnivo_calendar_color", true);
                            if($color=="" && strtoupper($box_bg_color)!="00A27C")
                                $color = $box_bg_color;
                            $hover_color = get_post_meta($events[key($events)]["id"], "omnivo_calendar_hover_color", true);
                            if($hover_color=="" && strtoupper($box_hover_bg_color)!="1F736A")
                                $hover_color = $box_hover_bg_color;
                            $text_color = get_post_meta($events[key($events)]["id"], "omnivo_calendar_text_color", true);
                            if($text_color=="" && strtoupper($box_txt_color)!="FFFFFF")
                                $text_color = $box_txt_color;
                            $hover_text_color = get_post_meta($events[key($events)]["id"], "omnivo_calendar_hover_text_color", true);
                            ...
                            $hours_text_color = get_post_meta($events[key($events)]["id"], "omnivo_calendar_hours_text_color", true);
                            if($hours_text_color=="" && strtoupper($box_hours_txt_color)!="FFFFFF")
                                $hours_text_color = $box_hours_txt_color;
                            $hours_hover_text_color = get_post_meta($events[key($events)]["id"], "omnivo_calendar_hours_hover_text_color", true);
            ...
            if(((int)$hide_empty && !$row_empty) || !(int)$hide_empty)
            {
                ...
                if(!(int)$hide_hours_column)
                {
                    $output .= '<td class="omnivo_calendar_hours_column">
    ...
    {
        $output .= '<div class="omnivo_calendar small ' . ($colors_responsive_mode ? 'use_colors' : '') . ' ' . ((int)$desktop_list_view ? 'desktop' : '') . '">';
        $l = 0;
        foreach($weekdays as $weekday)
        {
            ...
            if(isset($event_hours_tt[$weekday_fixed_number]))
            {
				...
				<ul class="omnivo_calendar_items_list thin page_margin_top omnivo_calendar_clearfix' . (isset($mode) && $mode=='12h' ? ' mode12' : '') . '">';
                ...
                for($i=0; $i<$event_hours_count; $i++)
                {
                    ...
                    $classes_url = "";
                    $omnivo_calendar_custom_url = get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_custom_url", true);
                    if(!(int)get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_disable_url", true) && !(int)$disable_event_url)
                        $classes_url = ($omnivo_calendar_custom_url!="" ? $omnivo_calendar_custom_url : get_permalink($event_hours_tt[$weekday_fixed_number][$i]["id"]));
                    ...
                    {
                        $color = get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_color", true);
                        if($color=="" && strtoupper($box_bg_color)!="00A27C")
                            $color = $box_bg_color;
                        $hover_color = get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_hover_color", true);
                        if($hover_color=="" && strtoupper($box_hover_bg_color)!="1F736A")
                            $hover_color = $box_hover_bg_color;
                        $text_color = get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_text_color", true);
                        if($text_color=="" && strtoupper($box_txt_color)!="FFFFFF")
                            $text_color = $box_txt_color;
                        $hover_text_color = get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_hover_text_color", true);
                        if($hover_text_color=="" && strtoupper($box_hover_txt_color)!="FFFFFF")
                        {
                            $hover_text_color = $box_hover_txt_color;
                            if($text_color=="")
                        ...
                        $hours_text_color = get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_hours_text_color", true);
                        if($hours_text_color=="" && strtoupper($box_hours_txt_color)!="FFFFFF")
                            $hours_text_color = $box_hours_txt_color;
                        $hours_hover_text_color = get_post_meta($event_hours_tt[$weekday_fixed_number][$i]["id"], "omnivo_calendar_hours_hover_text_color", true);
                        if($hours_hover_text_color=="" && (strtoupper($box_hours_hover_txt_color)!="FFFFFF" || $hours_text_color!=""))
                        {
                            $hours_hover_text_color = $box_hours_hover_txt_color;
                            if($hours_text_color=="")
                    ...
                    $output .= '
							<li ' . $colors_html . ' class="omnivo_calendar_clearfix">
                    ...
                    if(in_array($event_description_responsive, array("description-2", "description-1", "description-1-and-description-2")) && ($event_hours_tt[$weekday_fixed_number][$i]["before_hour_text"] || $event_hours_tt[$weekday_fixed_number][$i]["after_hour_text"]))
                    {
                        $output .= '<span class="event_description" ' . $text_colors_html . '>'.
                            (in_array($event_description_responsive, array("description-1", "description-1-and-description-2")) ? '<span class="event_description_1">'.$event_hours_tt[$weekday_fixed_number][$i]["before_hour_text"].'</span>' : '') .
                            ...
                            (in_array($event_description_responsive, array("description-2", "description-1-and-description-2")) ? '<span class="event_description_2">'.$event_hours_tt[$weekday_fixed_number][$i]["after_hour_text"].'</span>' : '') .
                            '</span>';
                    }
...
function omnivo_calendar_get_row_content($events, $args)
{
    extract($args);
    $content = "";
    foreach($events as $key=>$details)
    {
        ...
        {
            $color = get_post_meta($details["id"], "omnivo_calendar_color", true);
            $hover_color = get_post_meta($details["id"], "omnivo_calendar_hover_color", true);
        ...
        $text_color = get_post_meta($details["id"], "omnivo_calendar_text_color", true);
        if($text_color=="" && strtoupper($global_colors["box_txt_color"])!="FFFFFF")
            $text_color = $global_colors["box_txt_color"];
        $hover_text_color = get_post_meta($details["id"], "omnivo_calendar_hover_text_color", true);
        ...
        $hours_text_color = get_post_meta($details["id"], "omnivo_calendar_hours_text_color", true);
        ...
        $hours_hover_text_color = get_post_meta($details["id"], "omnivo_calendar_hours_hover_text_color", true);
        ...
        $omnivo_calendar_custom_url = get_post_meta($details["id"], "omnivo_calendar_custom_url", true);
        $classes_url = "";
        if(!(int)get_post_meta($details["id"], "omnivo_calendar_disable_url", true) && !(int)$disable_event_url)
            $classes_url = ($omnivo_calendar_custom_url!="" ? $omnivo_calendar_custom_url : get_permalink($details["id"]));
        ...
        {
            $tooltip = "";
            $content .= '<div class="event_container id-' . $details["id"] . (count(array_filter(array_values($details['tooltip']))) && (count($events)>1 || (count($events)==1 && $hours_count>1)) ? ' omnivo_calendar_tooltip' : '' ) . '"' . ($color!="" || ($text_color!="" && (count($events)>1 || (count($events)==1 && $hours_count>1))) ? ' style="' . ($color!="" ? 'background-color: #' . $color . ';' : '') . ($text_color!="" && (count($events)>1 || (count($events)==1 && $hours_count>1)) ? 'color: #' . $text_color . ';' : '') . '"': '') . (($hover_color!="" || $hover_text_color!="" || $hours_hover_text_color!="") && (count($events)>1 || (count($events)==1 && $hours_count>1)) ? ' onMouseOver="' . ($hover_color!="" ? 'this.style.background=\'#'.$hover_color.'\';' : '') . ($hover_text_color!="" ? 'this.style.color=\'#'.$hover_text_color.'\';jQuery(this).find(\'.event_header\').css(\'cssText\', \'color: #'.$hover_text_color.' !important\');' : '') . ($hours_hover_text_color!="" ? 'jQuery(this).find(\'.hours\').css(\'color\',\'#'.$hours_hover_text_color.'\');' : '') . '" onMouseOut="' . ($hover_color!="" ? 'this.style.background=\'#'.$color.'\';' : '') . ($hover_text_color!="" ? 'this.style.color=\'#'.$text_color.'\';jQuery(this).find(\'.event_header\').css(\'cssText\',\'color: #'.$text_color.' !important\');' : '') . ($hours_hover_text_color!="" ? 'jQuery(this).find(\'.hours\').css(\'color\',\'#'.$hours_text_color.'\');' : '') . '"' : '') . '>';
            ...
            if($tooltip!="")
            {
                ...
                if($hover_color=="" && strtoupper($global_colors["box_hover_bg_color"])!="1F736A")
                    $hover_color = $global_colors["box_hover_bg_color"];
                $content .= '<div class="omnivo_calendar_tooltip_text"><div class="omnivo_calendar_tooltip_content"' . ($hover_color!="" || $hover_text_color!="" ? ' style="' . ($hover_color!="" ? 'background-color: #' . $hover_color . ';' : '') . ($hover_text_color!="" ? 'color: #' . $hover_text_color . ';' : '') . '"': '') . '>' . $tooltip . '</div><div class="omnivo_calendar_tooltip_arrow"' . ($hover_color!="" ? ' style="border-color: #' . $hover_color . ' transparent;"' : '') . '></div></div>';
            }
            if(count($events)>1 || (count($events)==1 && $hours_count>1))
...
function omnivo_calendar_get_rowspan_value($hour, $array, $rowspan, $measure, $hours_min)
{
    ...
    if(!$found)
        ...
        return omnivo_calendar_get_rowspan_value($hour, $array, $rowspan, $measure, $hours_min);
}
...
function omnivo_calendar_hour_in_array($hour, $array, $measure, $hours_min)
{
    $array_count = count($array);
    for($i=0; $i<$array_count; $i++)
    {
...
function omnivo_calendar_strtolower_urlencode($val)
{
...
function omnivo_calendar_vc_init()
{
    ...
    vc_add_shortcode_param('dropdownmulti' , 'omnivo_calendar_vc_dropdownmultiple_settings_field');
    ...
    $omnivo_calendar_shortcodes_list = get_option('omnivo_calendar_shortcodes_list');
    $omnivo_calendar_shortcodes_array = array(__('choose...', 'omnivo_calendar') => '-1');
    if(!empty($omnivo_calendar_shortcodes_list))
    {
        foreach($omnivo_calendar_shortcodes_list as $key=>$val)
            $omnivo_calendar_shortcodes_array[$key] = $key;
    }
    ...
    vc_map(array(
        "name" => __("Calendar", 'omnivo_calendar'),
        "base" => "omnivo_calendar",
        "class" => "",
        "controls" => "full",
        "show_settings_on_create" => true,
        "icon" => "icon-wpb-layer-omnivo_calendar",
        "admin_enqueue_js"  => array(OMNIVO_CALENDAR_URL.'/assets/admin/js/omnivo_calendar_vc.js'),
        "front_enqueue_js"  => array(OMNIVO_CALENDAR_URL.'/assets/admin/js/omnivo_calendar_vc.js'),
        "params" => array(
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Choose shortcode id:", "omnivo_calendar"),
                "param_name" => "shortcode_id",
                "value" => $omnivo_calendar_shortcodes_array,
            ),
            array(
                "type" => "dropdownmulti",
                "class" => "",
                "heading" => __("Events", "omnivo_calendar"),
                "param_name" => "event",
                "value" => $events_array,
            ),
            array(
                "type" => "dropdownmulti",
                "class" => "",
                "heading" => __("Event categories", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdownmulti",
                "class" => "",
                "heading" => __("Hour categories", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdownmulti",
                "class" => "",
                "heading" => __("Columns", "omnivo_calendar"),
                "param_name" => "columns",
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Hour measure", "omnivo_calendar"),
                "param_name" => "measure",
                "value" => array(
                    __("Hour (1h)", "omnivo_calendar") => "1",
                    __("Half hour (30min)", "omnivo_calendar") => "0.5",
                    __("Quarter hour (15min)", "omnivo_calendar") => "0.25",
                ),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Filter style", "omnivo_calendar"),
                "param_name" => "filter_style",
                "value" => array(
                    __("Dropdown list", "omnivo_calendar") => "dropdown_list",
                    __("Tabs", "omnivo_calendar") => "tabs",
                ),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Filter kind", "omnivo_calendar"),
                "param_name" => "filter_kind",
                "value" => array(
                    __("By event", "omnivo_calendar") => "event",
                    __("By event category", "omnivo_calendar") => "event_category",
                    __("By event and event category", "omnivo_calendar") => "event_and_event_category",
                ),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Filter label", "omnivo_calendar"),
                "param_name" => "filter_label",
                "value" => __("All Events", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Filter label 2", "omnivo_calendar"),
                "param_name" => "filter_label_2",
                "value" => __("All Events Categories", "omnivo_calendar"),
                "dependency" => array(
                    "element" => "filter_kind",
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Select time format", "omnivo_calendar"),
                "param_name" => "select_time",
                "value" => array(
                    __("09.03 (H.i)", "omnivo_calendar") => "H.i",
                    __("09:03 (H:i)", "omnivo_calendar") => "H:i",
                    __("9:03 am (g:i a)", "omnivo_calendar") => "g:i a",
                    __("9:03 AM (g:i A)", "omnivo_calendar") => "g:i A",
                ),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Time format", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Hide 'All Events' view", "omnivo_calendar"),
                "param_name" => "hide_all_events_view",
                "value" => array(
                    __("No", "omnivo_calendar") => "0",
                    __("Yes", "omnivo_calendar") => "1",
                ),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Hide first (hours) column", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Show end hour in first (hours) column", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Event block layout", "omnivo_calendar"),
                "param_name" => "event_layout",
                "value" => array(
                    __("Type 1", "omnivo_calendar") => "1",
                    __("Type 2", "omnivo_calendar") => "2",
                    __("Type 3", "omnivo_calendar") => "3",
                    __("Type 4", "omnivo_calendar") => "4",
                    __("Type 5", "omnivo_calendar") => "5",
                ),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Hide empty rows", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Disable event url", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Text align", "omnivo_calendar"),
                "param_name" => "text_align",
                "value" => array(
                    __("center", "omnivo_calendar") => "center",
                    __("left", "omnivo_calendar") => "left",
                    __("right", "omnivo_calendar") => "right",
                ),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Id", "omnivo_calendar"),
                "param_name" => "id",
                "value" => "",
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Row height (in px)", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Display list view on desktop", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Responsive", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Event description in responsive mode", "omnivo_calendar"),
                "param_name" => "event_description_responsive",
                "value" => array(
                    __("None", "omnivo_calendar") => "none",
                    __("Only Description 1", "omnivo_calendar") => "description-1",
                    __("Only Description 2", "omnivo_calendar") => "description-2",
                    __("Description 1 and Description 2", "omnivo_calendar") => "description-1-and-description-2",
                ),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Collapse event hours in responsive mode", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Use colors in responsive mode", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Export to PDF button", "omnivo_calendar"),
            ...
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Generate PDF label", "omnivo_calendar"),
                "param_name" => "generate_pdf_label",
                "value" => __("Generate PDF", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("PDF Font", "omnivo_calendar"),
                "param_name" => "pdf_font",
                "value" => array(
                    __("Lato", "omnivo_calendar") => "lato",
                    __("DejaVu Sans", "omnivo_calendar") => "dejavusans",
                ),
            ),
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Calendar box background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Calendar box hover background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Calendar box text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Calendar box hover text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Calendar box hours text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Calendar box hours hover text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Filter control background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Row 1 style background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Row 2 style background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Generate PDF button text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Generate PDF button background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Generate PDF button hover text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Generate PDF button hover background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Booking button text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Booking button background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Booking button hover text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Booking button hover background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Booked button text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Booked button background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Unavailable button text color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Unavailable button background color", "omnivo_calendar"),
            ...
            array(
                "type" => "colorpicker",
                "class" => "",
                "heading" => __("Available slots color", "omnivo_calendar"),
            ...
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Table header font", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("or choose Google font", "omnivo_calendar"),
                "param_name" => "font",
            ...
            array(
                "type" => "dropdownmulti",
                "class" => "",
                "heading" => __("Google font subset", "omnivo_calendar"),
                ...
                "dependency" => array(
                    "element" => "font",
                    "not_empty" => true,
                    "callback" => "omnivo_calendar_font_subset_init",
                ),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Font size (in px)", "omnivo_calendar"),
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Show booking button", "omnivo_calendar"),
                "param_name" => "show_booking_button",
                "value" => array(
                    __("No", "omnivo_calendar") => "no",
                    __("Always", "omnivo_calendar") => "always",
                    __("On hover", "omnivo_calendar") => "on_hover",
                ),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Show available slots", "omnivo_calendar"),
            ...
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Available slots singular label", "omnivo_calendar"),
                "param_name" => "available_slots_singular_label",
                "value" => "{number_available}/{number_total} slot available",
                "description" => __("Specify text label for 'slot available' information (singular). Available placeholders: {number_available}, {number_taken}, {number_total}.", "omnivo_calendar")
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Available slots plural label", "omnivo_calendar"),
                "param_name" => "available_slots_plural_label",
                "value" => "{number_available}/{number_total} slots available",
                "description" => __("Specify text label for 'slots available' information (plural). Available placeholders: {number_available}, {number_taken}, {number_total}.", "omnivo_calendar")
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Default booking view", "omnivo_calendar"),
                "param_name" => "default_booking_view",
                "value" => array(
                    __("User", "omnivo_calendar") => "user",
                    __("Guest", "omnivo_calendar") => "guest",
                ),
                "description" => __("Specify which booking view should be visible by default.", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Allow user booking", "omnivo_calendar"),
                "param_name" => "allow_user_booking",
                "value" => array(
                    __("Yes", "omnivo_calendar") => "yes",
                    __("No", "omnivo_calendar") => "no",
                ),
                "description" => __("Set to 'Yes' if you want to allow logged in users to make a booking.", "omnivo_calendar"),
                "dependency" => array(
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Allow guest booking", "omnivo_calendar"),
                "param_name" => "allow_guest_booking",
                "value" => array(
                    __("No", "omnivo_calendar") => "no",
                    __("Yes", "omnivo_calendar") => "yes",
                ),
                "description" => __("Set to 'Yes' if you want to allow guests to make a booking.", "omnivo_calendar"),
                /*"dependency" => array(
                    "element" => "default_booking_view",
                    "value" => array("user"),
                ),*/
            ...
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Show guest name field", "omnivo_calendar"),
                ...
                "description" => __("Set to 'Yes' if you want to show 'Name' field in guest booking form.", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Guest name field required", "omnivo_calendar"),
                ...
                "description" => __("Set to 'Yes' if the 'Name' field should be required.", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Show guest phone field", "omnivo_calendar"),
                ...
                "description" => __("Set to 'Yes' if you want to show 'Phone' field in guest booking form.", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Guest phone field required", "omnivo_calendar"),
                ...
                "description" => __("Set to 'Yes' if the 'Phone' field should be required.", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Show guest message field", "omnivo_calendar"),
                ...
                "description" => __("Set to 'Yes' if you want to show 'Message' field in guest booking form.", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Guest message field required", "omnivo_calendar"),
                ...
                "description" => __("Set to 'Yes' if the 'Message' field should be required.", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Booking label", "omnivo_calendar"),
                "param_name" => "booking_label",
                "value" => __("Book now", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Booked label", "omnivo_calendar"),
                "param_name" => "booked_label",
                "value" => __("Booked", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Unavailable label", "omnivo_calendar"),
                "param_name" => "unavailable_label",
                "value" => __("Unavailable", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Popup booking label", "omnivo_calendar"),
            ...
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Popup login label", "omnivo_calendar"),
                "param_name" => "login_popup_label",
                "value" => __("Log in", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Popup cancel label", "omnivo_calendar"),
                "param_name" => "cancel_popup_label",
                "value" => __("Cancel", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Popup continue label", "omnivo_calendar"),
                "param_name" => "continue_popup_label",
                "value" => __("Continue", "omnivo_calendar"),
            ),
            array(
                "type" => "dropdown",
                "class" => "",
                "heading" => __("Terms and conditions checkbox", "omnivo_calendar"),
                "param_name" => "terms_checkbox",
                "value" => array(
                    __("No", "omnivo_calendar") => "no",
                    __("Yes", "omnivo_calendar") => "yes",
                ),
                "description" => __("Set to 'Yes' if you want to display 'Terms and conditions' checkbox.", "omnivo_calendar"),
            ),
            array(
                "type" => "textfield",
                "class" => "",
                "heading" => __("Terms and conditions message", "omnivo_calendar"),
                "param_name" => "terms_message",
                "value" => __("Please accept terms and conditions", "omnivo_calendar"),
                "description" => __("Specify text for 'Terms and conditions' checkbox.", "omnivo_calendar"),
            ),
            array(
                "type" => "textarea",
                "class" => "",
                "heading" => __("Booking pop-up message", "omnivo_calendar"),
                "param_name" => "booking_popup_message",
                "value" => BOOKING_POPUP_MESSAGE,
                "description" => __("Specify text that will appear in pop-up window. Available placeholders: {event_title} {column_title} {event_start} {event_end} {event_description_1} {event_description_2} {user_name} {user_email} {omnivo_calendar_btn_book} {omnivo_calendar_btn_cancel} {omnivo_calendar_btn_continue}", 'omnivo_calendar'),
            ),
            array(
                "type" => "textarea",
                "class" => "",
                "heading" => __("Booking pop-up thank you message", "omnivo_calendar"),
                "param_name" => "booking_popup_thank_you_message",
                "value" => BOOKING_POPUP_THANK_YOU_MESSAGE,
                "description" => __("Specify text that will appear in pop-up window. Available placeholders: {event_title} {column_title} {event_start} {event_end} {event_description_1} {event_description_2} {user_name} {user_email} {omnivo_calendar_btn_continue}", 'omnivo_calendar'),
            ),
            array(
                "type" => "textarea",
                "class" => "",
                "heading" => __("Custom CSS", "omnivo_calendar"),
...
add_action('init', 'omnivo_calendar_vc_init');

function omnivo_calendar_vc_dropdownmultiple_settings_field($settings, $value)
{
...
function prepare_booking_button($args)
{
    ...
    $output = '';
    $booking_url = '';
    if($args['omnivo_calendar_page_id']>0)
        $booking_url = get_permalink($args['omnivo_calendar_page_id']) . '#book-event-hour-' . $args['event_hours_id'];
