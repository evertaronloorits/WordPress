...
jQuery(document).ready(function($){
    function omnivoCalendarFields()
    {
        return {
            ...
            font_custom: {
                default: "",
                selector: "#omnivo_calendar_font_custom",
                type: "textfield",
            },
            font: {
                default: "",
                selector: "#omnivo_calendar_font",
                type: "font",
            },
            font_size: {
                default: "",
                selector: "#omnivo_calendar_font_size",
                type: "textfield",
            },
            show_booking_button: {
                default: "no",
            ...
            custom_css: {
                default: "",
                selector: "#omnivo_calendar_custom_css",
                type: "textarea",
            },
        };
    }

	var omnivo_calendar_fields = omnivoCalendarFields();
	
	if($("#omnivo_calendar_settings").length)
		$("#omnivo_calendar_settings")[0].reset();
	$(".omnivo_calendar_shortcode").val($(".omnivo_calendar_shortcode").data("default"));
	//events hours
...
//						$("#event_hours_table #add_event_hours").after("<input type='hidden' id='event_hours_id' name='event_hours_id' value='" + id + "' />");
	...
	$("#event, #event_category, #hour_category, #weekday, #measure, #filter_style, #filter_kind, #omnivo_calendar_settings [name='time_format'], #omnivo_calendar_settings [name='time_format_custom'], #hide_hours_column, #hide_all_events_view, #show_end_hour, #hide_empty, #disable_event_url, #text_align, #desktop_list_view, #responsive, #event_description_responsive, #collapse_event_hours_responsive, #colors_responsive_mode, #export_to_pdf_button, #pdf_font, #direction, #event_layout, #omnivo_calendar_font, #omnivo_calendar_font_subset, #show_booking_button, #show_available_slots, #allow_user_booking, #allow_guest_booking, #default_booking_view, #show_guest_name_field, #guest_name_field_required, #show_guest_phone_field, #guest_phone_field_required, #show_guest_message_field, #guest_message_field_required, #terms_checkbox").change(function(event, param){
		if(param=="skip")
			return;
		if($(this).attr("id")!="omnivo_calendar_font" || ($(this).attr("id")=="omnivo_calendar_font" && param==null))
			generateShortcode();
	});
	$("#filter_label, #filter_label_2, #generate_pdf_label, #available_slots_singular_label, #available_slots_plural_label, #booking_label, #booked_label, #unavailable_label, #booking_popup_label, #login_popup_label, #cancel_popup_label, #continue_popup_label, #terms_message, #row_height, #id, #omnivo_calendar_font_custom, #omnivo_calendar_font_size, #booking_popup_message, #booking_popup_thank_you_message, #omnivo_calendar_custom_css").on('change keyup', function(){
		generateShortcode();
	});
	
	$(".omnivo_calendar_shortcode").on("paste change", function(event){
		$("#omnivo_calendar_settings")[0].reset();
		$(".fontSubsetRow").css("display", "none").find("#omnivo_calendar_font_subset option").remove();
		...
		setTimeout(function(){
			...
			$(".omnivo_calendar_shortcode").not(self).val(shortcode);
			...
			shortcode = shortcode.replace("[omnivo_calendar ", "");
			shortcode = shortcode.substring(0, shortcode.lastIndexOf("]"));
			var attributes = shortcode.split(re2);
			
			var omnivo_calendar_atts = omnivoCalendarAtts();
			for(var i=0; i<attributes.length; i++)
			{
				for(var prop in omnivo_calendar_atts) {
					if(!omnivo_calendar_atts.hasOwnProperty(prop))
						continue;
					var att = omnivo_calendar_atts[prop];
			...
			for(var prop in omnivo_calendar_fields) {
				if(!omnivo_calendar_fields.hasOwnProperty(prop) || typeof(omnivo_calendar_atts[prop])=="undefined")
					continue;

				var field = omnivo_calendar_fields[prop],
					$field = $(field.selector);

				if(omnivo_calendar_atts[prop].val!=null)
				{
					if(["tinymce"].indexOf(field.type)!==-1)
						setTinyMCEContent(field.selector, omnivo_calendar_atts[prop].val);
					if(["textfield", "textarea", "dropdown"].indexOf(field.type)!==-1)
						$field.val(omnivo_calendar_atts[prop].val).trigger("change", ["skip"]);
					if(["font"].indexOf(field.type)!==-1)
						$field.val(omnivo_calendar_atts[prop].val).trigger("change", (omnivo_calendar_atts['font_subset'].val!=null ? [omnivo_calendar_atts['font_subset'].val.split(",")] : null));
					if(["time"].indexOf(field.type)!==-1)
					{
						$field.val(omnivo_calendar_atts[prop].val);
						$("[name='time_format'][value='" + omnivo_calendar_atts[prop].val + "']").prop("checked", true);
					}
					if(["multidropdown"].indexOf(field.type)!==-1)
						$field.val(omnivo_calendar_atts[prop].val.split(","));
					if(["colorpicker"].indexOf(field.type)!==-1)
						$field.val(omnivo_calendar_atts[prop].val).trigger("keyup", [1]);
				}
			}
		}, 1);
	});
	...
	$("#omnivo_calendar_settings [name='time_format']").change(function(){
		if($(this).val()!="custom")
		{
	...
	$("#omnivo_calendar_settings [name='time_format_custom']").on("focus", function(){
		$(this).prev().children().prop("checked", true);
	});
	$("#omnivo_calendar_settings [name='time_format_custom']").on("change", function(){
		var format = $(this).val();
...
//	$("#upcoming_events_time_from").live("change", function(){
	...
	$("#omnivo_calendar_configuration_tabs").tabs({
		create: function(event, ui) {
			$(this).removeClass('omnivo_calendar_hide');
		}
	});
	//filter label 2
	$("#filter_kind").change(function(event, param){
		...
		if(self.val()=="event_and_event_category")
			$(".filter_label_2").removeClass("omnivo_calendar_hide");
		else
			$(".filter_label_2").addClass("omnivo_calendar_hide");
	});
	...
	$("#default_booking_view").change(function(event, param) {
		...
		if(value=="user")
		{
			$("#allow_user_booking_wrapper").addClass('omnivo_calendar_hide');
			$("#allow_guest_booking_wrapper").removeClass('omnivo_calendar_hide');
		}
		else if(value=="guest")
		{
			$("#allow_guest_booking_wrapper").addClass('omnivo_calendar_hide');
			$("#allow_user_booking_wrapper").removeClass('omnivo_calendar_hide');
	...
	$("#allow_guest_booking").change(function(event, param){
		...
		if(self.val()=="yes")
		{
			$(".show_guest_name_field").removeClass("omnivo_calendar_hide");
			$(".guest_name_field_required").removeClass("omnivo_calendar_hide");
			$(".show_guest_phone_field").removeClass("omnivo_calendar_hide");
			$(".guest_phone_field_required").removeClass("omnivo_calendar_hide");
			$(".show_guest_message_field").removeClass("omnivo_calendar_hide");
			$(".guest_message_field_required").removeClass("omnivo_calendar_hide");
		}
		else
		{
			$(".show_guest_name_field").addClass("omnivo_calendar_hide");
			$(".guest_name_field_required").addClass("omnivo_calendar_hide");
			$(".show_guest_phone_field").addClass("omnivo_calendar_hide");
			$(".guest_phone_field_required").addClass("omnivo_calendar_hide");
			$(".show_guest_message_field").addClass("omnivo_calendar_hide");
			$(".guest_message_field_required").addClass("omnivo_calendar_hide");
		}
	});
	//font subset
	$(".google_font_chooser").change(function(event, param){
		...
		if(self.val()!="")
		{
			...
			$.ajax({
					url: ajaxurl,
					type: 'post',
					data: "action=omnivo_calendar_get_font_subsets&font=" + $(this).val(),
					success: function(data){
						data = $.trim(data);
						var indexStart = data.indexOf("omnivo_calendar_start")+15;
						var indexEnd = data.indexOf("omnivo_calendar_end")-indexStart;
	...
	//save event settings
	$("#omnivo_calendar_events_settings").on("submit", function(event) {
		event.preventDefault();
		var self = $(this);
		var spinner = self.find(".spinner");
		var events_slug = $("#omnivo_calendar_events_settings_slug").val();
		var events_label_singular = $("#omnivo_calendar_events_settings_label_singular").val();
		var events_label_plural = $("#omnivo_calendar_events_settings_label_plural").val();
		...
		$.ajax({
				url: ajaxurl,
				type: "post",
				data: "action=omnivo_calendar_ajax_events_settings_save&events_slug=" + events_slug + "&events_label_singular=" + events_label_singular + "&events_label_plural=" + events_label_plural,
				success: function(json){
					json = $.trim(json);
					spinner.css({
						"display": "none",
					...
					$event_slug_info.closest("tr").removeClass("omnivo_calendar_hide");
	...
	$("#edit_omnivo_calendar_shortcode_id").on("change", function(event) {
	    console.log($(this).val());
		$("#omnivo_calendar_shortcode_id").css({
			"background-color": "",
			"border": ""
		});
		$(".shortcode_info").css({
		...
		if($(this).val()!="-1")
		{
			...
			var shortcodeId = $("#edit_omnivo_calendar_shortcode_id :selected").text();
			$("#omnivo_calendar_shortcode_id").val(shortcodeId).trigger("paste");
			...
			var data = {
				'action': "omnivo_calendar_get_shortcode",
				'omnivo_calendar_shortcode_id': shortcodeId
			};
			$.ajax({
				url: ajaxurl,
				type: "post",
				...
				success: function(data){
					...
					if(data!==0)
					{
						...
						$(".omnivo_calendar_shortcode").val(data)
		...
		{
			$("input.omnivo_calendar_shortcode").val("[omnivo_calendar]").trigger("change")
			$("#shortcode_delete").css("display", "none");
			$("#omnivo_calendar_shortcode_id").val("");
		}
	});
	
	$("#omnivo_calendar_shortcodes").on("submit", function(event) {
		event.preventDefault();
		var self = $(this);
		var spinner = self.find(".spinner");
		var shortcodeId = $("#omnivo_calendar_shortcode_id").val();
		var shortcode = $("input.omnivo_calendar_shortcode").val();
		...
		if(!shortcode.length)
		{
			window.alert("Please make sure that calendar shortcode field isn't empty.");
			return;
		}
		var data = {
			'action': "omnivo_calendar_save_shortcode",
			'omnivo_calendar_shortcode_id': shortcodeId,
			'omnivo_calendar_shortcode': shortcode
		};
		$("#shortcode_delete").css("display", "none");
		spinner.css({
			"display": "inline-block",
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data!==0)
				{
					...
					if($("#edit_omnivo_calendar_shortcode_id option[value='" + shortcodeId + "']").length==0)
						$("#edit_omnivo_calendar_shortcode_id").append($("<option>", {
							value: shortcodeId,
							text: shortcodeId
						}));
					$("#edit_omnivo_calendar_shortcode_id").val(shortcodeId).trigger("change");
					$(".shortcode_info").css("display", "inline-block").html("Calendar shortcode saved.").fadeOut(3000);
				} else {
					console.log("error occured");
				}			
			}
	...
	$("#omnivo_calendar_shortcode_save1, #omnivo_calendar_shortcode_save2").on("click", function(event) {
		event.preventDefault();
		$("#omnivo_calendar_shortcodes").trigger("submit");
	})
	...
	$("#shortcode_delete").on("click", function(event) {
		...
		var data = {
			'action': "omnivo_calendar_delete_shortcode",
			'omnivo_calendar_shortcode_id': shortcodeId
		};
		//delete shortcode
		$.ajax({
			...
			success: function(data){
				...
				if(data!==0)
				{
					...
					$("#edit_omnivo_calendar_shortcode_id option[value='" + shortcodeId + "']").remove();
					$("#edit_omnivo_calendar_shortcode_id").val("-1").trigger("change");
					$(".shortcode_info").css("display", "inline-block").html("Calendar shortcode deleted.").fadeOut(3000);
					$("#shortcode_delete").css("display", "none");
					$("#omnivo_calendar_shortcode_id").val("");
				} else {
					console.log("error occured");
...
function generateShortcode()
{
	...
	$(".omnivo_calendar_shortcode").val("[omnivo_calendar" +
		params + 
		...
		($("#omnivo_calendar_settings [name='time_format']:checked").val()!='H.i' && $("#omnivo_calendar_settings [name='time_format']:checked").val()!='custom' ? " time_format='" + $("#omnivo_calendar_settings [name='time_format']:checked").val() + "'" : ($("#omnivo_calendar_settings [name='time_format']:checked").val()=="custom" && $("#omnivo_calendar_settings [name='time_format_custom']").val()!="H.i" ? " time_format='" + $("#omnivo_calendar_settings [name='time_format_custom']").val() + "'" : "")) + 
		...
		($("#omnivo_calendar_font_custom").val()!="" && $("#omnivo_calendar_font").val()=="" ? " font_custom='" + $("#omnivo_calendar_font_custom").val() + "'" : "") + 
		($("#omnivo_calendar_font").val()!="" ? " font='" + $("#omnivo_calendar_font").val() + "'" : "") + 
		($("#omnivo_calendar_font_subset").val()!=null ? " font_subset='" + $("#omnivo_calendar_font_subset").val().join() + "'" : "") + 
		($("#omnivo_calendar_font_size").val()!="" ? " font_size='" + $("#omnivo_calendar_font_size").val() + "'" : "") + 
		...
		($("#omnivo_calendar_custom_css").val()!="" ? " custom_css='" + $("#omnivo_calendar_custom_css").val().replace(/'/g, '"') + "'" : "") + 
	"]");
}
