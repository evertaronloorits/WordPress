jQuery(document).ready(function($){
	...
	var vc_omnivo_calendar_fields = vc_omnivo_calendar_fields();
	...
	$(document).ajaxComplete(function(event, xhr, settings) {
		...
		if(request_params.indexOf("tag=omnivo_calendar")!==-1 && request_params.indexOf("action=vc_edit_form")!==-1 && $(".vc_ui-panel-content .vc_shortcode-param select[name='shortcode_id']").val()!=-1)
		{
			vc_omnivo_calendar_load_settings(true);
		}
		else if(request_params.indexOf("tag=omnivo_calendar")!==-1 && request_params.indexOf("action=vc_edit_form")!==-1)
		{
	...
	$(document).on("change", ".vc_ui-panel-content .vc_shortcode-param select[name='font']", function(event, param) {
		...
		$.ajax({
			url: ajaxurl,
			type: "post",
			data: "action=omnivo_calendar_get_font_subsets&font=" + font,
	...
	$(document).on("change", ".vc_ui-panel-content .vc_shortcode-param select[name='shortcode_id']", function(event) {
		vc_omnivo_calendar_load_settings();
	});
	 ...
	 * @returns bool
	 */
	function vc_omnivo_calendar_load_settings(on_editor_load)
	{
		...
		if(!on_editor_load)
			vc_omnivo_calendar_settings_reset();
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data!==0)
					{
						...
						var omnivo_calendar_atts = omnivo_calendar_atts();
						...
						for(var prop in vc_omnivo_calendar_fields) {
							if(!vc_omnivo_calendar_fields.hasOwnProperty(prop) || typeof(omnivo_calendar_atts[prop])=="undefined")
								continue;
							
							var field = vc_omnivo_calendar_fields[prop],
							 ...
							 */
							if(omnivo_calendar_atts[prop].val!=null && ((!on_editor_load) || (on_editor_load && ($field.val()==null || $field.val()==field.default))))
							{
								if(["textfield", "textarea", "dropdown"].indexOf(field.type)!==-1)
									$field.val(omnivo_calendar_atts[prop].val).trigger("change");
	...
	function vc_omnivo_calendar_settings_reset()
	{
		for(var prop in vc_omnivo_calendar_fields) {
			if(!vc_omnivo_calendar_fields.hasOwnProperty(prop))
				continue;
			var field = vc_omnivo_calendar_fields[prop],
				$field = $(field.selector);
	...
	function vc_omnivo_calendar_fields()
	{
		/* 
...
function omnivo_calendar_font_subset_init() {
	var $ = jQuery;
